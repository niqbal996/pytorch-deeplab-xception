...
def main():
    ...
    for selection_count in range(100, len(lmdb_handle.image_paths), 500):
        for selection_iter in range(args.max_iterations):
            ...
            if os.path.exists(os.path.join(constants.RUNS, args.dataset, args.checkname, f'runs_{fraction_of_data_labeled:03d}', "selections")):
                ...
                model = DeepLab(num_classes=train_set.num_classes, backbone=args.backbone, output_stride=args.out_stride, sync_bn=args.sync_bn,
                                mc_dropout=(args.active_selection_mode.startswith('viewmc') or
                                            args.active_selection_mode.startswith('vote') or
                                            args.view_entropy_mode == 'mc_dropout'))
                model = model.cuda()
                # create trainer
                ...
                print('[INFO] Loading the best checkpoint . . .')
                ...
                print('[INFO] Saving the confusion matrix now . . .')
                ...
                print('[INFO] Making a new active selection for next run . . .')
                active_selector.select_next_batch(model, train_set, selection_count)
                # save selections
                print('[INFO] Saved the active selection into text file!')
