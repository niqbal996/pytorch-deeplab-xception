...
class GradientBasedSelector:
    ...
    def __init__(self, base_size, batch_size, num_classes):
        self.num_classes = num_classes
        self.base_size = base_size
        self.batch_size = batch_size
        self.scores = []

    def calculate_scores(self, model, loader):
        model.eval()
        model.cuda()
        self.scores = []

        # loader = DataLoader(PathsDataset(self.lmdb_handle, self.base_size, paths), batch_size=self.batch_size,
                ...
                for batch_idx in range(output.shape[0]):
                    ...
                    for c in range(self.num_classes):
                        entropy_map = entropy_map - (
                                    output[batch_idx, c, :, :] * torch.log2(output[batch_idx, c, :, :] + 1e-12))
                    entropy_map[label_batch[batch_idx, :, :] == 255] = 0
                    self.scores.append(entropy_map.mean().cpu().item())
                    del entropy_map
    ...
    def select_next_batch(self, model, training_set, selection_count):
        ...
        selected_samples = \
        list(zip(*sorted(zip(scores, training_set.remaining_image_paths), key=lambda x: x[0], reverse=True)))[1][
        :selection_count]
        model.eval()
